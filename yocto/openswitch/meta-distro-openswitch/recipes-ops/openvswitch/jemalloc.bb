...
FILES_${PN} = "/usr/lib/libjemalloc.a \
...
# Move libjemalloc shared libraries for target builds to $base_libdir
do_install_append_class-target() {
    if [ ${base_libdir} != ${libdir} ]
    then
        mkdir -p ${D}/${base_libdir}
        mv ${D}/${libdir}/libjemalloc.so.* ${D}/${base_libdir}
        libname=`readlink ${D}/${libdir}/libjemalloc.so`
        ln -sf ${@oe.path.relative("${libdir}", "${base_libdir}")}/$libname ${D}${libdir}/libjemalloc.so
    fi
}
