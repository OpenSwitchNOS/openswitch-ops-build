...
def get_static_analysis_cmd(d):
    ...
    if externalsrc:
        if os.path.isfile(os.path.join(d.getVar('TOPDIR', True), 'devenv-sca-enabled')):
            return "ops-devenv-fortify-sca"
    return ""

do_generate_sca_wrappers() {
    if which sourceanalyzer > /dev/null && echo "${MACHINE_FEATURES}" | grep -q ops-devenv-fortify-sca ; then
        sourceanalyzer -b ${PN} --clean
    else
        if [ -n "${EXTERNALSRC}" ] && echo "${MACHINE_FEATURES}" | grep -q ops-devenv-fortify-sca ; then
            echo
            echo
