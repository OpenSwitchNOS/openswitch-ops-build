...
def get_ops_config_symbols(d):
    config_file_path = d.getVar("BUILD_ROOT") + "/yocto/" + \
        d.getVar("DISTRO") + "/meta-platform-" + d.getVar("DISTRO") + \
        "-" + d.getVar("MACHINE") + "/.ops-config"
    ...
    # Create symlink to .ops-config if not already present
    first_link = d.getVar("TOPDIR") + "/.ops-config-" + d.getVar("MACHINE")
    second_link = d.getVar("TOPDIR") + "/.ops-config"
    if not os.path.isfile(first_link):
        os.symlink(config_file_path, first_link)
    if not os.path.isfile(second_link):
        os.symlink( first_link, second_link)
