...
EXTRA_OECONF = "--with-http_v2_module --with-http_ssl_module "
...
do_configure () {
  ...
  ./configure \
    ...
    --prefix=${prefix} --pid-path=${localstatedir}/run/nginx.pid \
    ${EXTRA_OECONF}
}
...
do_install () {
  ...
  install -m 0644 ${WORKDIR}/nginx-static.conf ${D}${sysconfdir}/nginx/conf.d/
  install -m 0644 ${WORKDIR}/nginx.service ${D}${sysconfdir}/nginx/
