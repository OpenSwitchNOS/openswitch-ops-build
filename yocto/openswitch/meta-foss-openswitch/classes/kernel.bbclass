...
kernel_do_install() {
	...
	install -d ${D}/boot
	install -m ${KERNEL_MODE} ${KERNEL_OUTPUT} ${D}/${KERNEL_IMAGEDEST}/${KERNEL_IMAGETYPE}-${KERNEL_VERSION}
...
kernel_do_deploy() {
	install -m ${KERNEL_MODE} ${KERNEL_OUTPUT} ${DEPLOYDIR}/${KERNEL_IMAGE_BASE_NAME}.bin
