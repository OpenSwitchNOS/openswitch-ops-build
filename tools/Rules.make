...
BASE_LINUX_FILE = $(BUILDDIR)/tmp/deploy/images/$(CONFIGURED_PLATFORM)/linux
...
BASE_EXT4_FILE = $(BUILDDIR)/tmp/deploy/images/$(CONFIGURED_PLATFORM)/$(DISTRO_FS_TARGET)-$(CONFIGURED_PLATFORM).ext4
...
devenv_clean: dev_header
	...
	$(V)rm -Rf src .devenv $(OPS_PACKAGE_GROUP_APPEND)
...
define DEVENV_ADD
	if ! grep -q '^$(1)$$' .devenv 2>/dev/null ; then \
	  ...
	  $(ECHO) "Adding $(1)-dbg package to the final image" ; \
          $(MAKE) -C $(BUILD_ROOT) _devenv_update_package_group_for_debug ; \
	else \
	  $(ECHO) "$(BLUE)$(1)$(GRAY) is already in your devenv" ; \
	fi ;
endef

OPS_PACKAGE_GROUP_RECIPE?=packagegroup-openswitch
OPS_PACKAGE_GROUP?=packagegroup-ops-base
OPS_PACKAGE_GROUP_APPEND=$(BUILDDIR)/workspace/appends/$(OPS_PACKAGE_GROUP_RECIPE).bbappend
.PHONY: _devenv_update_package_group_for_debug
_devenv_update_package_group_for_debug:
	$(V) echo -e "# Autogenerated file, do not modify\n\nRDEPENDS_$(OPS_PACKAGE_GROUP) += \"\\" > $(OPS_PACKAGE_GROUP_APPEND) ; \
	for package in `cat $(BUILD_ROOT)/.devenv` ; do \
	  echo "  $$package-dbg \\" >> $(OPS_PACKAGE_GROUP_APPEND) ; \
	done ; \
	echo "\"" >> $(OPS_PACKAGE_GROUP_APPEND)
...
devenv_rm: dev_header
	...
	$(V)rm -Rf src/$(PACKAGE)
	$(V)$(MAKE) _devenv_update_package_group_for_debug
